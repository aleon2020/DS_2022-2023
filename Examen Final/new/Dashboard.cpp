...
Dashboard::Dashboard(){
    conjuntoUsuarios.insert({"INVITADO", "AAAAAAAA"});
}
...
int Dashboard::entrarMenuRegistro(){
    ...
    for(auto it = conjuntoUsuarios.begin(); it != conjuntoUsuarios.end(); ++it){
        if(it->getNombreDeUsuario() == nombreDeUsuario && it->getNIF() == NIF){
            validar = true;
            registro.escribir(nombreDeUsuario, NIF);
        }
    }
    system("clear");
    if(validar == true){
